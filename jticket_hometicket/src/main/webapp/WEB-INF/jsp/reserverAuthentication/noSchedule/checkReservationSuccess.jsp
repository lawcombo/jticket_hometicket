...
<body>
   <div id="info-section" style="display:none;">
    ...
    <script>
		//팝업창에서 부모창으로 값을 돌려줌
		function returnReserverAuthenticatonValuesToParentWindow(result) {
			
			if(!window.opener != null && !window.opener.closed) {
				window.opener.setReturnedCheckReservationAuthenticationValuesFromChildWindow(result);
			}
			window.close();
		}
		...
		var result = {
			...
			name: '<%= sName %>',
			phone: '<%= sMobileNo %>',
			code: '<%= sResponseNumber %>',
		}
	
		setTimeout(function() {
			returnReserverAuthenticatonValuesToParentWindow(result);		
		}, 1000)
